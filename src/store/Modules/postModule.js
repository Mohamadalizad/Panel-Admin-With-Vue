...
export default {
  ...
  actions: {
    getPostById(context, id) {
      ...
      return axios.get(`api/post/${id}`).catch(err => {
        toast.error("عملیات به شکست برخورد کرد")
      }).finally(fin => {
    ...
    addPost(context, post) {
      context.commit("setLoading", true)
      return axios.post(`api/post`, post).catch(err => {
        toast.error("عملیات به شکست برخورد کرد")
      }).finally(fin => {
      context.commit("setLoading" , false)
      })
    ...
    editPost(context, post) {
      context.commit("setLoading", true)
      return axios.put(`api/post/`, post).catch(err => {
    ...
    deletePost(context, id) {
      context.commit("setLoading", true)
      return axios.delete(`api/post/${id}`).catch(err => {
        toast.error("عملیات به شکست برخورد کرد")
      }).finally(fin => {
      context.commit("setLoading" , false)
      })
